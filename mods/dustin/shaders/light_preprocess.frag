...
vec4 preproccess_tex(vec2 uv) {
    ...
    uv.x += sin(time + (uv.y*3.0)) * 0.001;
    uv.y += cos(time + (uv.x*3.0)) * 0.001;
...
vec4 crepuscular_rays(vec2 texCoords, vec2 pos) {
    float decay = 0.905*(1.0 + ((1.0-bright)/13.0));
    float density = .26;

    float weight = (0.16 * bright) + sin(time)*.01; 
	...
	{
        tc -= deltaTexCoord;
        vec4 sampl = preproccess_tex(tc.xy) * vec4(0.3);
...
void main()
{
    ...
    vec4 col = crepuscular_rays(uv, vec2(0.5+ cos(time+.30/2.0)*.04, 0.3 + sin(time/2.0)*.03));
    col *= 1.0;
