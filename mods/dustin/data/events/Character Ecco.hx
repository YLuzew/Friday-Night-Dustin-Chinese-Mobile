...
function create() {
    for (i=>strumLine in strumLines.members) {
        ...
        setupEccoDraw(i, strumLine.characters[0]);
    }
}

function setupEccoDraw(i:Int, char:Character) {
    eccoChars.set(i, char);
    char.onDraw = (basic:FlxSprite) -> {
...
function update(elapsed:Float) {
    for (i=>strumLine in strumLines.members) {
        if (strumLine.characters[0].onDraw == null) 
            setupEccoDraw(i, strumLine.characters[0]);
    }
}
