...
function create():Void {
    ...
    sText = new FlxText(0, 500, 500, "按图片\n以查看剧情", 40, true);
    ...
    exitText = new FunkinText(0, 660, FlxG.width, "< 离开", 46);
...
function update(elapsed:Float):Void {
    if (infoVisible && (FlxG.mouse.justPressed && FlxG.mouse.overlaps(exitText))) {
        cooldownTimer = 0.5;
        _hideInfo();
        return;
    }
    ...
    if (!infoVisible && (FlxG.mouse.justPressed && FlxG.mouse.overlaps(imageDisplay))) {
        cooldownTimer = 0.5;
        _showInfo();
    }
}
