...
function create() {
    ...
    for (i => song in FreeplaySonglist.get().songs) {
        var name = song.name.toLowerCase();
    ...
    for (i => song in FreeplaySonglist.get().songs) {
        ...
        var currentY = divider.y + divider.height + 20;
        ...
        if (creditData != null) {
            for (j in 0...creditLabels.length) {
                ...
                if (Reflect.hasField(creditData, field)) { 
                    ...
                    var labelMargin:Float = 20;
                    var valueMargin:Float = 30;  

                    // LABEL ITSELF
                    var labelTxt = new FunkinText(0, 0, boxBG.width, textLabel + ":", 30, false);
                    labelTxt.setFormat(Paths.font("8bit-jve.ttf"), 30, color);
                    labelTxt.setPosition(boxBG.x + labelMargin, currentY);
                    add(labelTxt);

                    // THE CREDITS THING
                    var valueTxtWidth = boxBG.width - valueMargin - 20;
...
function selectSong() {
    ...
    if (curPortrait.animation.name != null) {
            if (boxes[curSelected].song.name.toLowerCase() == "you-are" || boxes[curSelected].song.name.toLowerCase() == "you-are v2") {
